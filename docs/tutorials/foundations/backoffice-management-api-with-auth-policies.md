...
- **A genuinely stale reference survived the v18 migration.** `UmbracoCommunity.Extensions/Client/package.json`'s `generate-client` script still points at `https://localhost:44383/umbraco/swagger/umbracocommunityextensions/swagger.json` — the pre-v18 URL shape. Post-migration, that document is served under `/umbraco/openapi/...`, not `/umbraco/swagger/...`, so this script needs its URL updated before it'll work again; confirmed the old path is stale, though pinning down the *exact* replacement URL needs a running instance to verify against rather than a guess.
