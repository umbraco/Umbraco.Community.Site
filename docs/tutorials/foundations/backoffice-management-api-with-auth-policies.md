...
- **Versioning convention isn't identical across every sibling.** Block Restrictions, NotFoundTracker, and Blog Announcements all put `[ApiVersion("1.0")]` on the shared base controller; Extensions' base controller has no `[ApiVersion]` at all — each of *its* three concrete controllers carries the attribute individually instead. Both arrangements resolve the same `{version:apiVersion}` route token correctly (verified directly, not assumed), so nothing is broken — but a new Extensions controller that forgets the attribute would be, in a way the other three packages' shared-base convention doesn't risk.
One thing this tutorial found and fixed rather than just documented: `UmbracoCommunity.Extensions/Client/package.json`'s `generate-client` script had a genuinely stale reference that survived the v18 migration — it pointed at `https://localhost:44383/umbraco/swagger/umbracocommunityextensions/swagger.json`, the pre-v18 URL shape, which now 404s. Confirmed against a running instance that the document is actually served at `https://localhost:44383/umbraco/openapi/umbracocommunityextensions.json` (a real `200` with the expected `"title": "Umbraco Community Extensions Backoffice API"`), and updated the script accordingly.
