...
- **`drag-to-scroll-with-snap`** ([stub →](./foundations/drag-to-scroll-with-snap.md)) — the `<dc-slider>` component. Touch drag follows finger and snaps on release; desktop gets hover-zone navigation; explicit arrow buttons are an opt-in. Building a usable scroller in vanilla web components instead of pulling in Swiper / Embla. (When this gets written, fold in the `closest()`-based multi-host reuse trick as a short aside — `dc-slider` serves both the slider block and the blog showcase block via an opt-in `has-buttons` ancestor class, no host-type parameter. This was a standalone idea, pruned as too thin to stand alone.)
...
- **`github-oauth-member-authentication`** ([stub →](./foundations/github-oauth-member-authentication.md)) — wiring GitHub as the sole external login provider for Umbraco members. The non-obvious bits: `AddOAuth` vs. `AddRemoteScheme` (the latter skips `OAuthPostConfigureOptions` and null-refs in `OAuthHandler.BuildChallengeUrl`), fetching a verified private email via GitHub's `/user/emails` endpoint since the `email` OAuth scope only exposes the public one, and clearing all four Umbraco auth cookies via `IMemberSignInManager.SignOutAsync()` on sign-out rather than a plain `HttpContext.SignOutAsync()`. Umbraco's own docs cover Google/Microsoft OAuth; GitHub-as-member-login for a dev-community site is a natural but undocumented variant.
