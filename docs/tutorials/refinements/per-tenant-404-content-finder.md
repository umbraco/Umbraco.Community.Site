...
**The version that does work:** implement a resolver that Umbraco's 404 path will call, and resolve the tenant root from the **domain binding** that Umbraco already has on the incoming request.

## Our approach

This site uses the [`Umbraco.Community.NotFoundTracker`](../../../src/Umbraco.Community.NotFoundTracker/) package (a Razor Class Library that lives in this repo). It owns the request-pipeline plumbing — it hooks Umbraco's "about to serve a 404" path (the `IContentLastChanceFinder` slot Umbraco gives you for unmatched requests), records the miss for its dashboard, and asks a registered **`INotFoundPageResolver`** — an interface you implement — one question: *given this request, what content (if any) should I serve instead?* All we provide is the answer to that question, tenant-aware. (The site previously hand-rolled the whole thing as a bare `IContentLastChanceFinder`; moving to the package kept the same resolution logic and added 404 tracking on top — so if you find old references to `PageNotFoundContentFinder`, this is what replaced it.)

`INotFoundPageResolver` has a single method, `Task<IPublishedContent?> ResolveAsync(IPublishedRequestBuilder request)`. Return the content to serve (the package renders it and sets the 404 status); return `null` to let the package fall through to its default. The `IPublishedRequestBuilder` is Umbraco's wrapper around the in-flight request (here we only read from it), and it exposes `request.Domain?.ContentId` — the content ID of the root node bound to the request's domain. That's the tenancy signal you need: even though there's no current page, Umbraco *does* know which tenant the request belongs to, because it knew which domain was hit.

From there, the implementation is the foundation pattern in reverse: instead of `currentPage.Root()` → walk down, you have the root directly → walk down. Look for a `PageNotFound`-typed descendant under that root and hand it back. A fallback handles the case where there's no bound domain (local dev, IP-address requests, tests against the raw Kestrel host) — fall back to the first `PageNotFound` anywhere in the tree. This is one of the intentional cross-tenant lookups the foundation tutorial calls out.

## Walkthrough

The complete file lives at [`src/UmbracoCommunity.Web/Routing/CommunitySitePageNotFoundResolver.cs`](../../../src/UmbracoCommunity.Web/Routing/CommunitySitePageNotFoundResolver.cs).

### Step 1 — Implement `INotFoundPageResolver`
...
- **`IUmbracoContextAccessor`** to reach the published content cache. The `UmbracoContext` is Umbraco's per-request handle on the published content tree and the request itself; the *accessor* is the small interface you inject when you want to read that handle from anywhere outside an Umbraco render controller. We need it here because the resolver runs before any controller does.
...
- **`ILogger`** — the resolver logs which branch matched (handy when a tenant's 404 misbehaves).
...
The package and the resolver are wired up in one composer — a *composer* is an Umbraco-specific startup hook: any class that implements `IComposer` is picked up at boot and given a chance to register things with the DI container.
...
`AddNotFoundTracker()` registers the package (its pipeline hook, its 404-tracking store and dashboard). The second line registers *our* tenant-aware resolver as the `INotFoundPageResolver` the package will call — as a **singleton** (it's stateless, holding only its injected dependencies, so one shared instance is fine), which is why step 4 takes the `IServiceScopeFactory` route to reach scoped services. This composer gets picked up by `.AddComposers()` in `Program.cs` automatically (it implements `IComposer` and lives in an assembly Umbraco scans); no explicit `Program.cs` registration needed.

## Alternatives we considered

- **`app.UseStatusCodePagesWithReExecute("/404")`.** Mentioned above as an "obvious fix that doesn't work" — including here as the explicit comparison. The advantage is it's pure ASP.NET; you can use it without Umbraco-specific extension points. The disadvantage is everything that goes into making `/404` actually tenant-aware: per-tenant content paths, special-cased routing, status-code juggling between the re-execute and the response. By the time the plumbing's done, you've written a worse version of what the resolver gives you for free.
- **A `RenderController` action that takes over for any unmatched route.** Works for one tenant. For multiple, you'd need to inspect `HttpContext.Request.Host` inside the action to figure out which tenant's 404 to serve, then resolve the content yourself. The resolver is purpose-built for this and integrates with Umbraco's pipeline (preview mode, members, render-time block previews, …) for free.
...
- **A bare `IContentLastChanceFinder`** (what this site used before). Umbraco's built-in single-last-chance-finder hook works fine for serving the page, but it's just the serving half. The NotFoundTracker package wraps the same idea and adds a record of every 404 (with a backoffice dashboard and ignore rules), which is worth the dependency on a community site that wants to see what's 404ing.
...
- **One resolver.** The package calls a single `INotFoundPageResolver`. If you needed to chain logic — try a per-tenant 404, then a global one, then a "did you mean…" search — you'd build that chain inside the one resolver's `ResolveAsync`. Not worth it for this site; if your requirements grow that way, consider a small strategy pattern inside the resolver.
