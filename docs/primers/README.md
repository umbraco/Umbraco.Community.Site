...
- **[Third-party integrations primer](integrations.md)** — the external dependencies: three server-side data feeds (Sessionize, Calendar, Community Blogs) that all follow one resilient pattern (options + named `HttpClient` + cache-with-stale-fallback + composer), the frontend analytics layer (a production-only GTM server-side-tagging container) and a dormant, unmounted Cookiebot consent element, and the avatar/build-time bits. Notes what *isn't* here (Matomo, Intercom, Maps, release tracking) so readers don't go hunting.
