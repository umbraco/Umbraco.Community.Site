...
public class FetchRecentIssuesJob
{
    ...
    public async Task ExecuteAsync(PerformContext? context, string? specificRepo = null, CancellationToken cancellationToken = default)
    {
        ...
        context?.WriteLine($"Starting FetchRecentIssuesJob for repo: {repo}");
        ...
        try
        {
            ...
            context?.WriteLine("Fetching recent issues from GitHub...");
            var issues = await _apiClient.FetchRecentIssuesAsync(specificRepo, log, cancellationToken);

            context?.WriteLine($"Fetched {issues.Count} recent issues");
