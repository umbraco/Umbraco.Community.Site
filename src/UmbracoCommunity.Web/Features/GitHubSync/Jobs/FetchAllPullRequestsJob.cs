...
public class FetchAllPullRequestsJob
{
    ...
    public async Task ExecuteAsync(PerformContext? context, string? specificRepo = null, CancellationToken cancellationToken = default)
    {
        ...
        context?.WriteLine($"Starting FetchAllPullRequestsJob for repo: {repo}");
        ...
        try
        {
            ...
            context?.WriteLine("Fetching pull requests from GitHub...");
            var pullRequests = await _apiClient.FetchAllPullRequestsAsync(specificRepo, log, cancellationToken);

            context?.WriteLine($"Fetched {pullRequests.Count} pull requests");
