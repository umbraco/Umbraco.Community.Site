...
import type { GitHubHqMember, EmploymentPeriod } from "../api/types.gen.js";
...
export class HqMembersDashboardElement extends UmbElementMixin(LitElement) {
  ...
  async #onSave() {
    ...
    if (this._isCreating) {
      const { error } = await UmbracoCommunityExtensionsService.createHqMember({
      ...
      const { error } = await UmbracoCommunityExtensionsService.updateHqMember({
        path: { id: this._editingMember.login },
        body: this._editingMember,
      });
  ...
  async #onDelete(member: GitHubHqMember) {
    ...
    const { error } = await UmbracoCommunityExtensionsService.deleteHqMember({
  ...
  #onAddPeriod() {
    if (!this._editingMember) return;
    const newPeriod: EmploymentPeriod = { start: null, end: null };
    this._editingMember = {
      ...this._editingMember,
      periods: [...this._editingMember.periods, newPeriod],
    };
  }
          ...
          <div class="periods-section">
            ...
            ${this._editingMember.periods.length === 0
              ...
              : this._editingMember.periods.map(
                  (period: EmploymentPeriod, index: number) => html`
