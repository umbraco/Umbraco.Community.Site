...
import type { ContributionStats, ReleaseSummary, ContributorDetail, ReleaseInfo } from "../api/types.gen.js";
...
export class ContributionStatsDashboardElement extends UmbElementMixin(LitElement) {
  ...
  async #loadData() {
    ...
    try {
      const [contributionResult, releaseResult] = await Promise.all([
        UmbracoCommunityExtensionsService.getContributionStats({
          query: { startDate: this._startDate, endDate: this._endDate }
        }),
        UmbracoCommunityExtensionsService.getReleases({
          query: { startDate: this._startDate, endDate: this._endDate }
        })
      ]);

      if (contributionResult.error) {
        throw new Error(`Failed to load contributions: ${String(contributionResult.error)}`);
      }
      if (releaseResult.error) {
        throw new Error(`Failed to load releases: ${String(releaseResult.error)}`);
      }

      this._contributionStats = contributionResult.data as ContributionStats;
      this._releases = releaseResult.data as ReleaseSummary;
    } catch (error) {
  ...
  #renderContributionStats() {
    ...
    return html`
      <uui-box headline="External Contributions Summary (Umbraco CMS)">
        ...
        ${this._contributionStats.topContributors && this._contributionStats.topContributors.length > 0
          ? html`
              <div class="top-contributors">
                ...
                <uui-table>
                  ...
                  ${this._contributionStats.topContributors.map(
                    (contributor: ContributorDetail) => html`
                      <uui-table-row>
                        <uui-table-cell>
                          ${this.#renderGitHubLink(contributor.login ?? '', contributor.name ?? contributor.login ?? '')}
                        </uui-table-cell>
                        <uui-table-cell>
  ...
  #renderReleases() {
    ...
    const majorReleases = this._releases.releases?.filter((r: ReleaseInfo) => r.isMajor) ?? [];
    ...
    return html`
                  ...
                  <div class="release-section">
                    <div class="release-header">
                      <h3>
                        ...
                        ${this.#renderLtsBadge(release.isLts ?? false)}
                      </h3>
                    ...
                    ${release.topContributors && release.topContributors.length > 0
                      ? html`
                          <div class="release-contributors">
                            <strong>Top contributors:</strong>
                            ${release.topContributors.map(
                              (c: ContributorDetail) => html`
                                <a
                                ...
                                >
                                  ${c.name ?? c.login} (${c.pullRequestCount})
                                </a>
                              `
                            )}
                          </div>
      ...
      <uui-box headline="All Releases">
        ${this._releases.releases && this._releases.releases.length > 0
          ? html`
              <uui-table>
                <uui-table-head>
